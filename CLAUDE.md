...
**成長 (growth)**: `growthAbilityUnlocked` (ability→＋) and `growthSpellUnlocked` (additional spell card) are **separate** unlock flags (per rules — distinct 強化 choices), each toggled in PCCard's GM-edit panel. `activeAbility = growthAbilityUnlocked ? growthAbility : as`. (Full session-end 成長 ceremony — 弾幕スキル再修得・タグ獲得・特別な絆/親密度 — is a separate unbuilt feature; current toggles are the interim unlock.)
