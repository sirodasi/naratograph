...
The **session-end 成長 ceremony** (`GrowthCeremony` in `SessionView.jsx`, opened from `SessionEndView`'s 🌟 button): each player (GM: all PCs) chooses **成長** (both ok) — 弾幕スキル再修得 (pick from the session's appeared-character `ds` pool) + タグ獲得 (free text) — and **強化** (one of, each once-per-campaign) — 追加スペカ (`spell`) / 能力スキル＋ (`ability`) / 特別な絆 (`bond`, → `pc.specialBond = { target, targetUid, intimacy }`). Persisted to **Firebase `playerGrowth/{uid}/{charId}`** = `{ ds, tags[], enhancementsUsed[], specialBond }`; `Lobby.confirmChar` reads it back and writes the grown fields into the player record (tags = base+acquired, ds override, growth flags from `enhancementsUsed`, specialBond) so it carries across sessions (app rebuilds PCs each session). PC build in App.jsx carries `specialBond`. The 親密度 **live mechanics** (target's スペシャル +1 / 交流 +1D6, cheer-slot refresh per increase, 2 dice at 10) are **not yet wired** — ceremony + persistence only so far.
