...
7. **Epilogue** (`sessionPhase: "epilogue"`, `EpilogueView`): after the **final** battle (any outcome, `isFinal`), a 終幕 depiction shown to all before the end screen. Uses the **same scene-mode UI as explore's 描写** — `SceneStage` (bg + 立ち絵 + text) + GM-side `SceneEditor` (both exported from `SessionView.jsx`, also used by `MapView`/`RightPanel`); data flows through `sceneData` (`rooms/{roomCode}/scene`) + `gs.sceneText`. `finishBattle` routes `isFinal ? "epilogue" : "explore"`. **Free portrait placement**: each portrait is `{ img, name, x, y, h, flip }` (x,y = bottom-center %, h = height %; legacy `{img,name}` falls back to a centered-bottom spread). With `editable` (GM, when `setSceneData` is passed), `SceneStage` lets the GM **drag** portraits (local state during drag, commits to `sceneData` on pointer-up to avoid Firebase spam) and a per-portrait toolbar adjusts size/flip/layer/delete.
