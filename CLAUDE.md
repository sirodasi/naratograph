...
**Evasion-change effects** (`computeEvasion` in `declareSpell`): mutate the target's `resources.回避力.cur` (±1) so `getDefaultEvadeDice` picks it up without touching the evade step. The original value is saved in `gs.battle.evasionRestore[entityId]` and **restored in `handleCleanup`** (round-scoped) — **all four types including `costs_own_evasion`** (杞人地). Note: 回避力 recovers to max at round end by design, so 杞人地's "回避力 -1" is a current-value (not max) reduction that only bites within the declaring round (先攻→後攻 evade starts at 2 / 後攻→fewer moves that round). `computeEvasion(p)` returns `{ pcs, npcs, evRestore }`; the caller applies them to `p` before `mergeConsumeWithBattle` so the SC-consume and evasion-change coexist. Wired into all three placement paths (full-deterministic, full-random, designated-choose). **Still manual** (remaining): reset_graze, costs_rei, and all post-dodge / on-hit / round-end timing effects.
