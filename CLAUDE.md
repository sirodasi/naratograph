...
Background music: **GM uploads files → Firebase Storage → URL shared via `gs.bgm`, played locally per client** (not synced playback). In the 🎵 panel (`RightPanel`, GM only) the GM picks/drops an audio file per phase; `uploadBgm(phase, file)` (in `SessionApp`) uploads to `rooms/{roomCode}/bgm/{phase}` in Firebase Storage, gets the download URL, and writes it to `gs.bgm.{explore,battle,end}` via `upd()`. `clearBgm` deletes the Storage object and clears the field. Each client's `SessionApp` derives the active URL from phase (`gs.battle?.active` → battle, `sessionPhase === "end"` → end, else explore) and calls `bgm.setTrack(url)`. `bgm` ([src/bgm.js](src/bgm.js)) wraps two `HTMLAudioElement`s for **crossfade** on track change. Volume/mute are personal settings in `localStorage` (`bgmVolume`/`bgmMuted`, **default muted**). Browser autoplay policy: `bgm.unlock()` is called on the first `pointerdown` (so `play()` runs inside a user gesture); before unlock, the desired track is remembered and started on unlock. Invalid URLs / autoplay rejections are swallowed silently.

**Firebase Storage** is initialized in [firebase.js](src/firebase.js) (`export const storage`). BGM upload requires Storage **security rules** allowing authenticated writes under `rooms/{roomCode}/bgm/` — if uploads fail, that's the likely cause (the UI surfaces a "Storage設定を確認" error).
