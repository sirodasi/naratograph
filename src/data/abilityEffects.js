...
export const ABILITY_EFFECTS = {
  ...
  // 風を操る（射命丸文）：新聞表を振り直す（新聞モーダルの「風: 振り直す」ボタンで対応）。
  "風を操る程度の能力":   { note: "新聞表を振り直す（やる気1消費・新聞モーダルで自動対応）" },
  "風を操る程度の能力＋": { note: "新聞表を振り直す（やる気消費なし・新聞モーダルで自動対応）" },
  // 空を飛ぶ（博麗霊夢・オート）：自分が表（アイテム/変調/ペナルティ/ハプニング/手がかりイベント）を
  // 振った直後、振り直しプロンプトが出る（animateDice に統合・1日1回・pc.soraFlewDay で管理）。
  "空を飛ぶ程度の能力":   { passive: true, note: "1日1回、表を振った直後に全ダイスを振り直せる（自動：振り直しプロンプト）" },
  "空を飛ぶ程度の能力＋": { passive: true, note: "1日1回、表を振った直後に全ダイスを振り直せる（行為判定以外の他処理はGM）" },
  // 正体を判らなくする（封獣ぬえ）：自分を対象に選べなくする（pc.untargetable トグル。対象選択ピッカーが除外）。
  "正体を判らなくする程度の能力":   { freq: null, auto: true, kind: "toggle_untargetable", note: "1日1回、特殊効果の対象に選べなくする（base・回数管理はGM）" },
  "正体を判らなくする程度の能力＋": { freq: null, auto: true, kind: "toggle_untargetable", note: "回数制限なし" },
  ...
  // 老いることも死ぬこともない（藤原妹紅）：決戦以外で不死身の霊力消費を3点(＋2点)に軽減（不死身ボタンの immortalCost）
